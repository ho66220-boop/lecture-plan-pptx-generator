...
`TARGET_MONTH`에 월(숫자)을 넣으면 **정규반 강좌만** 진도표에 그 달 + **다음 달 미리보기**(회색·"예정")
까지 함께 보여주되, 회차·수강기간·수강료는 **그 달 기준**으로 재계산합니다(다음 달은 표시용이라 청구에 안 들어감).
전체 기간 계획서를 만들려면 `None`으로 둡니다. (입력칸 `input()` 대신 변수라 셀 재실행에 안전합니다.)
